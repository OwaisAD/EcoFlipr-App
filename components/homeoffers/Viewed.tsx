...
export const Viewed = ({ user }: ViewedProps) => {
  ...
  const fetchRandomOffers = useCallback(async () => {
    try {
      ...
      await AsyncStorage.setItem("cachedOffersViewed", JSON.stringify(offersData));
  ...
  const loadCachedOffers = useCallback(async () => {
    try {
      setLoading(true);
      const cachedOffers = await AsyncStorage.getItem("cachedOffersViewed");
      if (cachedOffers) {
