...
export const Recent = ({ user }: RecentProps) => {
  ...
  const fetchRandomOffers = useCallback(async () => {
    try {
      setLoading(true);
      const offersQuery = query(collection(db, "saleoffers"), limit(100)); // Adjust the limit as needed
      ...
      const offersData = offersSnapshot.docs
        ...
        .sort((a, b) => {
          return (
            new Date(formatFirebaseDate(b.createdAt)).getTime() - new Date(formatFirebaseDate(a.createdAt)).getTime()
          );
        });

      // Save fetched offers to AsyncStorage
      await AsyncStorage.setItem("cachedOffersRecent", JSON.stringify(offersData));
  ...
  const loadCachedOffers = useCallback(async () => {
    try {
      setLoading(true);
      const cachedOffers = await AsyncStorage.getItem("cachedOffersRecent");
      if (cachedOffers) {
